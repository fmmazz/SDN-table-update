...
measure_rules = 1
counter = 1
initial_priority = 1000 + initial_rules - 1
measure_priority = 1000
...
class FirstExperiment(app_manager.RyuApp):
	...
	def send_flow_mod(self, datapath):
		...
		for k in xrange(1,2001):
			...
			actions = [ofp_parser.OFPActionOutput(ofp.OFPP_NORMAL, 2)]
		...
		initial_rules = initial_rules + 500
	
		if initial_rules != 0:
			for k in xrange(initial_rules, initial_rules-500, -1):
		...
		if initial_rules != 0:
			for k in xrange(initial_rules-501, 0, -1):
				...
				req = ofp_parser.OFPFlowMod(datapath, cookie, cookie_mask,
								...
								match, inst)
				datapath.send_msg(req)
				if k == initial_rules-501:
					burst[ipv4_dst_first] = time.time()

		print "Installing initial rules"
	...
	def end_measurement(self):
		...
		w = open('Modify' + str(initial_rules-500) + '.txt','a')
